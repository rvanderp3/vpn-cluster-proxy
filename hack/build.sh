...
LDFLAGS="${LDFLAGS} " #-X github.com/rvanderp/vpn-cluster-proxy-webhook/pkg/version.Raw=${GIT_TAG} -X github.com/rvanderp/vpn-cluster-proxy-webhook/pkg/version.Commit=${GIT_COMMIT}"
TAGS="${TAGS:-}"
OUTPUT="${OUTPUT:-bin/vpn-cluster-proxy-webhook}"
...
go build "${GOFLAGS}" -ldflags "${LDFLAGS}" -tags "${TAGS}" -o "${OUTPUT}" ./cmd/vpn-cluster-proxy-webhook
